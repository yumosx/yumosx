## Hi there, I’m Ian Wang (@yumosx) 👋

Gopher • OpenTelemetry CNCF member • Open-source addict  

Currently spending most of my time making observability simpler, faster, and more fun in Go.

🔭 Active contributor of:
- [OpenTelemetry Go](https://github.com/open-telemetry/opentelemetry-go) (SDK + Auto-instrumentation)
- [Apache Dubbo-Go](https://github.com/apache/dubbo-go)

------
[![Contribution Stats](https://github-contribution-stats.vercel.app/api/?username=yumosx)](https://github.com/LordDashMe/github-contribution-stats/)
